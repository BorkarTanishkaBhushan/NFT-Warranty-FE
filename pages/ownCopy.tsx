...
async function getData() {
    ...
    await querySnapshot.forEach(async (doc) => {
        ...
        if (date - doc.data()["time_created"] >= doc.data()["period"] * 60000) {
            ...
            tempStatus = "Pending"
            let tempId = doc.id
            const docRef = doc(db, "warranty", tempId);
...
async function changeStatus(item: any) {
    const warrantyCollectionRef = collection(db, "warranty");
        const q = query(warrantyCollectionRef, where("tokenUri", "==", item.uri));
...
const filteredData = selectedStatus === 'All' ? final_Array : final_Array.filter((item : any) => item.status === selectedStatus);
  ...
  <div className="overflow-x-auto sm:-mx-6 lg:-mx-8 text-2xl">
    ...
    <table className="min-w-full text-left text-sm font-light">
                    ...
                    <tr>
                        <th
                        scope="col"
                        className="px-6 py-4 text-2xl text-[#16194F]"
                        >
                        Seller
                        </th>
                        <th
